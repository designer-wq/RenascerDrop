...
const AdminDistribution: React.FC = () => {
    ...
    const handleDistribute = (player: Player, action: 'Conquistado' | 'Passou') => {
        if (!currentItem) return;

        if (action === 'Conquistado') {
            // If player wins, everyone ABOVE them in priority gets a "Passou" automatically
            ...
            if (winnerIndex > 0) {
                ...
                skippedPlayers.forEach(skippedPlayer => {
                    ...
                    distributeItem(skippedPlayer.id, currentItem, 'Passou', false);
                });
            }
            // Mark winner and CONSUME item
            distributeItem(player.id, currentItem, 'Conquistado', true);
        } else {
            // If manually clicking Pass, just mark this player as passed and DO NOT consume item
            distributeItem(player.id, currentItem, 'Passou', false);
        }
    };
            ...
            <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mb-3">Gerenciar Distribuição</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-lg">Selecione um item abaixo para iniciar a distribuição.</p>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg border border-blue-100 dark:border-blue-800">
                        ...
                        <span className="text-sm font-bold">{items.length} Itens Cadastrados</span>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">1. Escolha o Item</label>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex flex-col">
                                ...
                                <p className="text-xs text-slate-500">Itens adicionados aguardando distribuição</p>
                            </div>
                            ...
                            {sortedPlayers.map((player) => (
                                <div key={player.id} className="group bg-surface-light dark:bg-surface-dark border border-slate-100 dark:border-stone-800 rounded-xl p-4 sm:p-5 shadow-sm hover:border-primary/50 transition-all flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                                    ...
                                    <div className="flex items-center w-full lg:w-auto gap-2">
                                        <button
                                            onClick={() => handleDistribute(player, 'Conquistado')}
                                        ...
                                        <button
                                            onClick={() => handleDistribute(player, 'Passou')}
                ...
                <div className="mt-12 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-100 dark:border-red-900/30 p-6">
                    ...
                    <p className="text-sm text-red-500 mb-6">As ações abaixo são irreversíveis. Tenha certeza antes de clicar.</p>

                    <div className="grid sm:grid-cols-3 gap-4">
                        <button
                            onClick={() => {
                                if (window.confirm("ATENÇÃO: Isso excluirá TODOS os jogadores cadastrados. Tem certeza?")) {
                                    clearPlayers();
                                }
                            }}
                        ...
                        <button
                            onClick={() => {
                                if (window.confirm("ATENÇÃO: Isso excluirá TODOS os itens cadastrados. Tem certeza?")) {
                                    clearItems();
                                }
                            }}
                            className="px-4 py-3 bg-white dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-600 font-bold rounded-xl hover:bg-red-600 hover:text-white transition-all flex items-center justify-center gap-2"
                        ...
                        <button
                            onClick={() => {
                                if (window.confirm("ATENÇÃO: Isso limpará TODO o histórico de loot. Tem certeza?")) {
                                    clearHistory();
                                }
                            }}
                            className="px-4 py-3 bg-white dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-600 font-bold rounded-xl hover:bg-red-600 hover:text-white transition-all flex items-center justify-center gap-2"
