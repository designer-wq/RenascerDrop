...
const ItemRegistration: React.FC = () => {
    ...
    const rarities: Rarity[] = ['Lendário', 'Épico', 'Raro', 'Incomum', 'Comum'];

    const getRarityColor = (r: Rarity) => {
        switch (r) {
            case 'Lendário': return 'bg-orange-500 border-orange-500 text-white';
            case 'Épico': return 'bg-purple-600 border-purple-600 text-white';
            case 'Raro': return 'bg-blue-500 border-blue-500 text-white';
            case 'Incomum': return 'bg-green-500 border-green-500 text-white';
    ...
    const getRarityBadgeColor = (r: Rarity) => {
        switch (r) {
            case 'Lendário': return 'bg-orange-500/80';
            case 'Épico': return 'bg-purple-500/80';
            case 'Raro': return 'bg-blue-500/80';
            case 'Incomum': return 'bg-green-500/80';
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            // Check original file size first (just a soft check, we will compress anyway if needed)
            ...
            reader.onload = (readerEvent) => {
                ...
                img.onload = () => {
                    ...
                    // Compress to JPEG with quality 0.7 to ensure small size
                    // Base64 string length ~ 4/3 * size_in_bytes.
                    // 1MB limit = ~750KB bytes.
                    // Quality 0.7 usually results in much smaller files than 750KB for 800px images.
                    const dataUrl = canvas.toDataURL('image/jpeg', 0.7);

                    // Final safety check (approximate base64 length check)
                    // 1MB = 1048576 characters approx.
                    if (dataUrl.length > 1000 * 1024) {
                        alert("A imagem ainda é muito grande após compressão. Tente uma imagem com menos detalhes ou resolução menor.");
                        return;
                    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (name && stats && chance && cost) {
                ...
                alert("Por favor, preencha todos os campos obrigatórios (Nome, Atributos, Chance, Custo).");
            }
        } catch (error) {
            ...
            <div className="w-full max-w-[1400px] mx-auto px-4 lg:px-8 py-8 space-y-12">
                <header className="flex flex-col gap-2">
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-[#1c130d] dark:text-white tracking-tight">
                        {editingId ? 'Editar' : 'Cadastro de'} <span className="text-primary">Itens</span>
                    </h1>
                    <p className="text-text-muted dark:text-gray-400 max-w-2xl">
                        Adicione novos itens ao sistema. Eles aparecerão automaticamente no Dashboard e estarão disponíveis para distribuição.
                    </p>
                </header>
                    ...
                    <div className="flex-1 w-full bg-surface-light dark:bg-surface-dark rounded-2xl shadow-card border border-border-light dark:border-border-dark p-6 md:p-8 relative overflow-hidden">
                        {success && (
                            <div className="absolute top-0 left-0 w-full bg-green-500 text-white text-center py-2 font-bold text-sm animate-fade-in-down">
                                {editingId ? 'Item atualizado com sucesso!' : 'Item cadastrado com sucesso!'}
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-6 mt-2">
                            ...
                            <div className="flex flex-col md:flex-row gap-4">
                                <div className="flex-1 space-y-2">
                                    <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Nome do Item</label>
                                    <input
                                        type="text"
                                        value={name}
                                ...
                                <div className="w-full md:w-1/3 space-y-2">
                                    <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Custo (Garnet)</label>
                                    <input
                                        type="number"
                                        value={cost}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Raridade</label>
                            ...
                            <div className="flex flex-col md:flex-row gap-4">
                                <div className="flex-1 space-y-2">
                                    <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Atributos (Resumo)</label>
                                    <input
                                        type="text"
                                        value={stats}
                                ...
                                <div className="w-full md:w-1/3 space-y-2">
                                    <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Chance de Drop</label>
                                    <input
                                        type="text"
                                        value={chance}
                            ...
                            {/* Image Upload */}
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-text-main dark:text-gray-200 ml-1">Imagem do Item</label>
                                ...
                                {!iconUrl ? (
                                    <div className="relative group w-full h-32 rounded-xl bg-gray-50 dark:bg-black/20 border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-primary dark:hover:border-primary transition-colors cursor-pointer flex flex-col items-center justify-center gap-2">
                                        ...
                                        <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Clique para selecionar uma imagem</span>
                            ...
                            <div className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-700">
                                ...
                                <label className="cursor-pointer select-none text-sm font-bold text-text-main dark:text-white" onClick={() => setLimitToTop5(!limitToTop5)}>
                                    Limitar distribuição aos Top 5
                                </label>
                                <div className="group relative ml-auto">
                                    ...
                                    <div className="absolute bottom-full right-0 mb-2 w-64 rounded-lg bg-gray-900 p-3 text-xs text-white opacity-0 shadow-xl transition-opacity group-hover:opacity-100 pointer-events-none z-10">
                                        Se ativado, este item aparecerá apenas para os 5 primeiros jogadores da fila de prioridade (baseado em CP/DKP).
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="w-full xl:w-[400px] flex flex-col gap-4">
                        ...
                        <article className="flex flex-col gap-4 p-5 rounded-2xl bg-surface-light dark:bg-surface-dark border border-gray-100 dark:border-gray-800 shadow-xl transition-all duration-300">
                            ...
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-text-muted">Custo Estimado</span>
                        ...
                        <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 text-blue-700 dark:text-blue-300 text-sm">
                            ...
                            Ao salvar, este item ficará disponível na lista global e poderá ser selecionado pelo Admin para distribuição em raids.
                        </div>
                    </div>
                </div>
