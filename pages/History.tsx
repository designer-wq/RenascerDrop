...
const History: React.FC = () => {
    ...
    const totalDistributed = lootHistory.filter(e => e.status === 'Conquistado').length;
    const filteredCount = filteredHistory.length;

    const getItemIcon = (itemId: string) => items.find(i => i.id === itemId)?.iconUrl || '';
    const getItemRarity = (itemId: string) => items.find(i => i.id === itemId)?.rarity || 'Comum';
    ...
    return (
        <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-8 px-4 py-8 md:px-10">
            ...
            <div className="flex flex-col gap-3">
                <label className="text-sm font-bold text-text-muted uppercase px-1">Filtrar por Item</label>
            ...
            <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 flex flex-col gap-2">
                    <label className="text-sm font-bold text-text-muted uppercase">Filtrar por Jogador</label>
                ...
                {/* Date Filter */}
                <div className="flex-1 flex flex-col gap-2">
                    <label className="text-sm font-bold text-text-muted uppercase">Filtrar por Data</label>
                    <input
                        type="date"
                        value={selectedDate}
                ...
                <div className="flex-1 flex flex-col gap-2">
                    ...
                    <div className="flex bg-gray-50 dark:bg-black/20 p-1 rounded-xl border border-gray-200 dark:border-gray-700 h-12">
                        ...
                        <button
                            onClick={() => setStatusFilter('Conquistado')}
                            className={`flex-1 rounded-lg text-sm font-bold transition-all ${statusFilter === 'Conquistado' ? 'bg-white dark:bg-gray-700 shadow-sm text-primary' : 'text-gray-500 hover:text-text-main'}`}
                        >
                            Wins
                        </button>
                        <button
                            onClick={() => setStatusFilter('Passou')}
                            className={`flex-1 rounded-lg text-sm font-bold transition-all ${statusFilter === 'Passou' ? 'bg-white dark:bg-gray-700 shadow-sm text-red-500' : 'text-gray-500 hover:text-text-main'}`}
                        >
                            Passes
                        </button>
                    </div>
            ...
            {filteredHistory.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center text-gray-400 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl">
                    ...
                    <h3 className="text-lg font-bold text-text-main dark:text-gray-300">Nenhum registro encontrado</h3>
                    <p className="max-w-xs mx-auto mt-2">Tente ajustar os filtros para encontrar o que procura.</p>
                </div>
            )}
                        ...
                        <div key={event.id} className="group flex flex-col md:flex-row items-start md:items-center gap-6 p-4 rounded-2xl bg-surface-light dark:bg-surface-dark border border-gray-100 dark:border-gray-800 hover:border-primary/30 hover:shadow-lg transition-all">
                            ...
                            >
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] uppercase font-bold text-gray-500">Jogador</span>
                                </div>
                            </div>
                            ...
                            <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 ${event.status === 'Conquistado'
                                ...
                                <span className="material-symbols-outlined text-sm">
                                    {event.status === 'Conquistado' ? 'emoji_events' : event.status === 'Passou' ? 'close' : 'help'}
                                </span>
                                {event.status}
                            </div>
                            ...
                            <div className="flex items-center gap-4 w-full md:w-auto p-3 rounded-xl bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-gray-700">
                                ...
                                <div className="flex flex-col">
                                    <span className={`text-sm font-bold ${rarity === 'Lendário' ? 'text-orange-500' :
                                        rarity === 'Épico' ? 'text-purple-500' :
